...
public partial class CokeRefillInteractable : Area3D
{
	...
	public void OnInteract()
	{
		...
		GetTree().CreateTimer(2.0f).Timeout += () => {
			...
			// INAYOS: Nilagyan ng protektadong bracket para sa sakop ng if statement
			...
			dm.StartDialogue(new string[] { "John: Done. Wait... sino yung nakatayo sa labas?" }, () => {
				...
				GetTree().CreateTimer(StalkerLingerTime).Timeout += () => {
					...
					// INAYOS: Ginamit muli ang 'dm' variable imbes na gumawa pa ng 'dm2'
					dm.StartDialogue(new string[] { "John: Hm. Sige na lang. Kumuha na ko ng walis." }, () => {
						if (Broom != null) Broom.IsActive = true;
						
						// Awtomatikong mag-uupdate ang iyong To-Do list UI
						GetNode<TodoManager>("/root/TodoManager").UpdateTodoText("Get Broom");
					});
				};
			});
		};
