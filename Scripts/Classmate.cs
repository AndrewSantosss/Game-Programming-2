...
public partial class Classmate : Node3D
{
    ...
    public override void _PhysicsProcess(double delta)
    {
        if (_isWalking)
        {
            ...
            Vector3 newVelocity = (nextPathPos - currentPos).Normalized() * _walkSpeed;

            if (newVelocity.Length() > 0.1f)
            {
                Vector3 lookTarget = new Vector3(nextPathPos.X, GlobalPosition.Y, nextPathPos.Z);
                LookAt(lookTarget, Vector3.Up);
            }

            GlobalPosition += newVelocity * (float)delta;
        }
    }
            ...
            if (!_firstDialogueDone)
            {
                ...
                GetNode<DialogueManager>("/root/DialogueManager").StartDialogue(lines, () => {
                    ...
                    if (CoffeeShopTarget != null) {
                        MoveTo(CoffeeShopTarget.GlobalPosition, () => {
                            _reachedCoffeeShop = true;
                            GD.Print("Reached the Coffee Shop!");
                        });
                    }
                }); // Added closing for StartDialogue
            } // Added closing for if (!_firstDialogueDone)
